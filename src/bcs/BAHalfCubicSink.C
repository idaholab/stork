...
BAHalfCubicSink::computeQpJacobian()
{
  ...
  if (x >= 0)
    ...
    return full_value*6*x*(x - _cutoff)/_cutoff3*_phi[_j][_qp]*_dpp_dv[_qp][_pvar][_pvar];
}
...
BAHalfCubicSink::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  if (x >= 0)
    ...
    return full_value*6*x*(x - _cutoff)/_cutoff3*_phi[_j][_qp]*_dpp_dv[_qp][_pvar][dvar];
}
